...
install() {
    bash <(curl -Ls https://raw.githubusercontents.com/ldsj005/XrayR-script/master/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update() {
    ...
    bash <(curl -Ls https://raw.githubusercontents.com/ldsj005/XrayR-script/master/install.sh) $version
    if [[ $? == 0 ]]; then
...
update_shell() {
    wget -O /usr/bin/XrayR -N --no-check-certificate https://raw.githubusercontents.com/ldsj005/XrayR-script/master/XrayR.sh
    if [[ $? != 0 ]]; then
        echo ""
...
--- https://github.com/ldsj005/XrayR ---
  ${green}0.${plain} 修改配置
————————————————
